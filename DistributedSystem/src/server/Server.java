...
public class Server {
    ...
    public Server() {
        String dir = "serverfile";
        try {
            serverMasterSocket = new Socket("localhost", serverMasterPort);
            clientServerSocket = new ServerSocket(serverClientPort);        // server1
            masterServerSocket = new ServerSocket(serverFile2MasterPort);   // server1
            checkFileSocket = new ServerSocket(serverFile2MasterCheckPort); // server1
            ...
            */
            ...
            serverBlock.setServerID(1);
            serverBlock.setServerName("localhost");
            serverBlock.setPortClient(serverClientPort);            // server1
            serverBlock.setPortMaster(serverFile2MasterPort);       // server1
            serverBlock.setPortClientCheck(clientFile2ServerPort);  // server1
        ...
        public void run() {
            try {
                ...
                while (true) {
                    ...
                    if(line != null) {
                        File f = new File("serverfile\\"+line.trim());
                        if(f.exists()) {
                            dos.writeUTF("ok");
                ...
                // e.printStackTrace();
                System.err.println("Master Fail");
                System.exit(-5);
            } finally {
                try {
                    if (dis != null)
                ...
                while(true) {
                    ...
                    for (String i : items) {
                        ...
                        if (!f.exists()) {
                            ...
                            String ff = dis.readUTF();
                            long fLength = dis.readLong();
                            FileOutputStream fos = new FileOutputStream(new File("serverfile\\" + ff));
                            System.out.println("Starting receiving file <" + ff + ">, size of file = " + fLength);
                            byte[] receiveBytes = new byte[1024];
                            int transLen = 0;
            ...
            try {
                ...
                if (type.equalsIgnoreCase("download")) {
                    ...
                    while(true) {
                        ...
                        dos2Master.flush();
                        File fff = new File("serverfile\\" + f);
