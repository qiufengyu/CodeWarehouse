...
#ifdef DEBUG
    printf("---next piece is %d (least first)---\n",min_index);
    fprintf(nfp, "---next piece is %d (least first)---\n",min_index);
#endif
	fflush(nfp);
	fclose(nfp);
    return min_index;
...
int select_next_subpiece(int index,int *begin,int *length){
    ...
    list_foreach(ptr,&downloading_piece_head){
        ...
        if(d_piece->index == index){
			fprintf(nfp, "-- next pieve is index: %d (not first least)-- \n");
			fflush(nfp);
	fclose(nfp);
...
#ifdef DEBUG
						printf("next subpiece is index:%d begin:%d length:%d\n",index,*begin,*length);
                       // fprintf(nfp, "next subpiece is index:%d begin:%d length:%d\n",index,*begin,*length);
    	//fflush(nfp);
	//fclose(nfp);
#endif
                        my_unlock(download_mutex);
