...
FILE *createfile(char *filepath, int size){
    ...
    if (!exists(filepath)){
    	...
    	for(; filepath[k] != 0;k++)
    		;
    	if(filepath[0] == '.')
    		memcpy(&path_tmp,filepath,k);
    	else{
    		memcpy(&path_tmp[1],filepath,k);	
    		printf("re\b\n");
    		path_tmp[0] = '.';
    		//path_tmp[1] = '/';
    		memcpy(&dir[1], filepath, i+1);
    		dir[0] = '.';
        ...
        int fd = creat(path, 0644);//int fd = creat(filepath, 0644);
        // printf("1\n");
        if (fd == -1){
            int tmp = errno;            
            printf("%s\n", strerror(tmp));
...
FILE *createfile(char *filepath, int size){
    if (!exists(filepath)){
        ...
        int fd = creat(filepath, 0644);//int fd = creat(filepath, 0644);
        printf("1\n");
        if (fd == -1){
            int tmp = errno;            
            printf("%s\n", strerror(tmp));
