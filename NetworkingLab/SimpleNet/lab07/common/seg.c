...
int sip_sendseg(int connection, seg_t* segPtr)
{
	...
	printf("Sending ......\n");
	printf("src_port: %d\n", segPtr->header.src_port);        //源端口号
	printf("dest_port: %d\n", segPtr->header.dest_port);       //目的端口号
	printf("seq_num: %d\n", segPtr->header.seq_num);         //序号
	printf("ack_num: %d\n", segPtr->header.ack_num);         //确认号
	printf("length: %d\n", segPtr->header.length);    //段数据长度
	printf("type: %d\n", segPtr->header.type);     //段类型
	printf("---------------------------------\n");
		...
		switch(state) {
			...
			case SEGSTART2: {
				if(recv_buf == '&') {
					int n = 0;
					//Attention the type!
					if((n = recv(connection, (char *)&segPtr->header, sizeof(stcp_hdr_t)+segPtr->header.length,0))>0) {
